...
void main() async {
  ...
  runApp(VietCoinAIApp(initialToken: token));
}

class VietCoinAIApp extends StatelessWidget {
  final String? initialToken;
  
  const VietCoinAIApp({super.key, this.initialToken});
    ...
    return MaterialApp(
      ...
      theme: ThemeData(
        colorScheme: ColorScheme.fromSeed(
          seedColor: const Color(0xFFB7791F), // Màu vàng đồng thương hiệu
          brightness: Brightness.dark, // VietCoin AI dùng Dark Mode
        ),
        useMaterial3: true,
      ),
...
class _WebViewScreenState extends State<WebViewScreen> {
  ...
  void _initWebView() {
    _controller = WebViewController()
      ...
      ..setBackgroundColor(const Color(0xFF121212)) // Dark background for loading
  ...
  NavigationDecision _handleNavigation(NavigationRequest request) {
    ...
    if (url.startsWith(AppConfig.webBaseUrl) || url.contains('10.0.2.2')) {
      return NavigationDecision.navigate;
    }
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            const Icon(Icons.cloud_off_rounded, size: 80, color: Color(0xFFB7791F)),
            const SizedBox(height: 24),
            const Text('Mất kết nối Internet', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
