...
async def check_payment_status(payment_id: int, user=Depends(get_current_user)):
    ...
    # Sau khi sync mà vẫn pending, kiểm tra xem có quá hạn (1 phút) chưa
    if payment["status"] == "pending":
...
def mark_payment_failed(payment_id: int, user=Depends(get_current_user)):
    db = UserDB()
    payment = db.get_payment(payment_id)
    if not payment:
        db.close()
    ...
    if payment["status"] == "pending":
        db.update_payment_status(payment_id, "failed")
        status = "failed"
        print(f"🛑 Đơn hàng #{payment_id} đã được đóng/hủy thủ công bởi người dùng.")
    else:
        status = payment["status"]
        
    db.close()
    return {"message": "Đã cập nhật trạng thái thất bại", "status": status}
        ...
        async with httpx.AsyncClient() as client:
            ...
            for tx in transactions:
                ...
                if match:
                    ...
                    if tx_payment and tx_payment["status"] != "completed":
                        ...
                        db.update_payment_status(tx_payment_id, "completed")
...
def create_payment_report(
    ...
    if not payment:
        db.close()
        raise HTTPException(status_code=404, detail=f"Không tìm thấy hóa đơn #{payment_id} trong hệ thống")
    
    if payment["user_id"] != user["id"] and not user.get("is_admin"):
        db.close()
        raise HTTPException(status_code=403, detail="Bạn không có quyền báo cáo hóa đơn của người khác")
